...
export function StatusCards({ halted, premium }: { halted: boolean; premium: number }) {
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
      ...
      <Card>
        <div className="flex items-start justify-between">
          ...
          <div className={`flex items-center gap-1 text-xs text-mono ${elevated ? "text-warning" : "text-success"}`}>
            {elevated ? <Flame className="h-3.5 w-3.5" /> : <TrendingUp className="h-3.5 w-3.5" />}
            {elevated ? "ELEVATED" : "BASELINE"}
          </div>
        </div>
        <div className="mt-6 flex items-baseline gap-2">
          <span className="text-5xl font-semibold tracking-tight text-mono tabular-nums">{premium}</span>
      ...
      <Card tone={halted ? "emergency" : "default"}>
        <div className="flex items-start justify-between">
          ...
          <div className={`flex items-center gap-1 text-xs text-mono ${halted ? "text-emergency" : "text-success"}`}>
            {halted ? <AlertOctagon className="h-3.5 w-3.5" /> : <CheckCircle2 className="h-3.5 w-3.5" />}
        ...
        {halted ? (
          ...
          <>
            <div className="mt-6 text-xl font-semibold tracking-tight">Liquidity active</div>
            ...
            <div className="mt-5 grid grid-cols-2 gap-4 text-mono">
              <div>
                <div className="text-[10px] uppercase tracking-wider text-ink-muted">TVL</div>
                <div className="text-sm font-medium tabular-nums">2,418,902 CSPR</div>
              </div>
              <div>
                <div className="text-[10px] uppercase tracking-wider text-ink-muted">Util.</div>
      ...
      <Card>
        ...
        <div className="mt-6 text-[13px] text-ink-muted">Immutable contract hash</div>
        <button
          onClick={copy}
